...
public class LocationService implements PlacesServiceApi {
  ...
  private static void provisionCategories(@NonNull final GeocodeParameters parameters){
    ...
    categories.addAll(CategoryHelper.foodTypes);
  }
  ...
  private class GeocodeProcessor implements Runnable{
    ...
    GeocodeProcessor(final ListenableFuture<List<GeocodeResult>> results, final PlacesServiceCallback callback){
      mCallback = callback;
      mResults = results;
    }
  ...
  private class RouteSolver implements Runnable{
    ...
    RouteSolver(final Point start, final Point end, final RouteServiceCallback callback){
      ...
      if (status == LoadStatus.FAILED_TO_LOAD) {
        ...
        routeTaskFuture.addDoneListener(() -> {
          try {
            ...
            routeResFuture.addDoneListener(() -> {
              try {
