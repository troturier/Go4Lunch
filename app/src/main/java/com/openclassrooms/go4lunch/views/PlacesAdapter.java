...
public class PlacesAdapter extends RecyclerView.Adapter<RestaurantViewHolder> {
    ...
    public PlacesAdapter(final Context context, final List<Place> places, RequestManager glide, Listener callback){
        mPlaces = places;
        PlacesAdapter.glide = glide;
    ...
    @Override public final void onBindViewHolder(final RestaurantViewHolder holder, final int position) {
        ...
        holder.updateWithResult(place, listId.get(position), listOpenNow.get(position), this.glide, this.callback);
    }
